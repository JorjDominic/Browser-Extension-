(() => {
  ...
  function showUrlScanCard() {
    // Don't show on Shopee product pages (handled by content.js)
    const isShopee = location.hostname.includes("shopee.");
    const isProductPage = isShopee && /-i\.\d+\.\d+/.test(location.href);
    
    if (isProductPage) {
      console.log("Universal: Skipping URL scan card for Shopee product page");
      return;
    }
  ...
  function notifyBackgroundScript() {
    ...
    if (isShopee && isProductPage) {
      // Let the Shopee-specific content script handle this - NO URL SCAN
      console.log("Universal: Shopee product page detected, letting content.js handle");
      return;
    } else {
      ...
      // ALL other pages including Shopee non-product pages - URL scanning
      console.log("Universal: Sending URL_SCAN_PAGE message for:", location.href);
      
      // Show URL scan card that will display results
      showUrlScanCard();
  ...
  setInterval(() => {
    if (location.href !== lastUrl && isActiveTab) {
      console.log("Universal: URL changed from", lastUrl, "to", location.href);
